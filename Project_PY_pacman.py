...
def main():
    ...
    ready_word.rect.x = tile_size * 8 - tile_size / 2
    ready_word.rect.y = tile_size * 13 + tile_size // 4
    ...
    while running:
        if lives == 0:
            ...
            quontity_lives = pygame.sprite.Sprite()
            quontity_lives.image = load_image("pac man/closed.png")
            quontity_lives.rect = quontity_lives.image.get_rect()
            quontity_lives.rect.x = tile_size
            quontity_lives.rect.y = 782
            quontity_.add(quontity_lives)

            quontity_lives1 = pygame.sprite.Sprite()
            quontity_lives1.image = load_image("pac man/closed.png")
            quontity_lives1.rect = quontity_lives1.image.get_rect()
            quontity_lives1.rect.x = tile_size * 3
            quontity_lives1.rect.y = 782
            quontity_.add(quontity_lives1)
        elif lives == 2:
            quontity_lives1 = pygame.sprite.Sprite()
            quontity_lives1.image = load_image("out.png")
            quontity_lives1.rect = quontity_lives1.image.get_rect()
            quontity_lives1.rect.x = tile_size * 3
            quontity_lives1.rect.y = 782
            quontity_.add(quontity_lives1)
            ...
            quontity_lives = pygame.sprite.Sprite()
            quontity_lives.image = load_image("out.png")
            quontity_lives.rect = quontity_lives.image.get_rect()
            quontity_lives.rect.x = tile_size
            quontity_lives.rect.y = 782
            quontity_.add(quontity_lives)
        ...
        for i in range(x_size):
            for j in range(y_size):
                if cells_drawing[' '.join([str(i * tile_size), str(j * tile_size)])] == 'wall':
                ...
                elif cells_drawing[' '.join([str(i * tile_size), str(j * tile_size)])] == 'empty' and lines[j][
                    i] == '.':
                    pygame.draw.rect(screen, (255, 255, 255),
                                     (i * tile_size + 13, j * tile_size + 13, tile_size - 27, tile_size - 27))
                    if pacman.pos == [i * tile_size, j * tile_size] and lines[j][i] == '.':
                        zxc = list(lines[j])
                        zxc[i] = '/'
                        lines[j] = ''.join(zxc)
                        count += 100
                    ...
                    if pacman.pos == [i * tile_size, j * tile_size]:
                        zxc = list(lines[j])
                        zxc[i] = '/'
                        lines[j] = ''.join(zxc)
                        count += 1000
        font = pygame.font.Font(None, 50)
        texter = font.render(str(counter), True, 'white')
        screen.blit(texter, (0, 0))
        all_sprites.draw(screen)
        quontity_.draw(screen)
        if countdown < FPS * 3:
            ready.draw(screen)
        else:
            all_sprites.draw(screen)
    ...
    game_over.rect.x = tile_size * 6
    game_over.rect.y = tile_size * 13 + tile_size // 4
    over.add(game_over)
