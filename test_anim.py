    ...
    for x in range(x_size):
        for y in range(y_size):
            if lines[y][x] == "." or lines[y][x] == '/':
                ...
                cells_drawing[' '.join([str(x * tile_size), str(y * tile_size)])] = 'huh'
                ...
                pacman = PacmanSprite(all_sprites, load_image("sprites sheets/pacman/pacman sheet left.png"), 4, 1,
                                      x * tile_size, y * tile_size, tile_size, width, height)
        ...
        for i in range(x_size):
            for j in range(y_size):
                if cells_drawing[' '.join([str(i * tile_size), str(j * tile_size)])] == 'wall':
                    if lines[j][i] == '#':
                        pygame.draw.rect(screen, '#FF8C00', (i * tile_size, j * tile_size, tile_size, tile_size))
                    elif lines[j][i] == '+':
                        pygame.draw.rect(screen, (255, 100, 150),
                                         (i * tile_size, j * tile_size + 10, tile_size, tile_size - 30))
                if cells_drawing[' '.join([str(i * tile_size), str(j * tile_size)])] == 'huh':
                    pygame.draw.rect(screen, (0, 0, 0), (i * tile_size, j * tile_size, tile_size, tile_size))
                if cells_drawing[' '.join([str(i * tile_size), str(j * tile_size)])] == 'empty':
                    if lines[j][i] == '.':
                        pygame.draw.rect(screen, 'white',
                                         [tile_size * i + 15, tile_size * j + 15, tile_size - 30, tile_size - 30])
        for i in range(x_size):
            for j in range(y_size):
                if lines[j][i] == '.' and [tile_size * i, tile_size * j] == pacman.pos:
                    zxc = list(lines[j])
                    zxc[i] = '/'
                    lines[j] = ''.join(zxc)
                    count += 100
        font = pygame.font.Font(None, 50)
        texter = font.render(str(count), True, 'white')
